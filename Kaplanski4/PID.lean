...
lemma mem_map_constantCoeff {I : Ideal R⟦X⟧} {x : R} :
    x ∈ I.map constantCoeff ↔ ∃ f ∈ I, f.constantCoeff = x :=
...
theorem map_constantCoeff_subset_self : (C (R := R))'' I.map constantCoeff ⊆ I := by
  intro _ ⟨_, _, ha⟩
...
variable {S : Set R} (hSI : span S = I.map constantCoeff)
include hSI

theorem eq_span_insert_X : I = span (insert X (C '' S)) := by
  ext f
  rw [mem_span_insert, ← map_span, hSI]
  exact ⟨
    fun _ ↦ ⟨mk fun p ↦ coeff (p + 1) f, C f.constantCoeff,
...
variable {P : Ideal R⟦X⟧} (hP : X ∉ P) {k : ℕ} {a : Fin k → R}
  (haP : span (range a) = P.map constantCoeff)

/- Exctacting `f : Fin k → R⟦X⟧` from `a : Fin k → R` -/
section f

include haP in
private lemma exists_f : ∀ i, ∃ f ∈ P, f.constantCoeff = a i :=
...
private lemma hfa : ∀ i, (f haP i).constantCoeff = a i := fun i ↦ (exists_f haP i).choose_spec.2
...
include haP hg in
private lemma exists_r : ∃ r : Fin k → R, ∑ i, r i * a i = g.constantCoeff :=
  mem_span_range_iff_exists_fun.1 (haP ▸ constantCoeff_mem_map_of_mem ‹_›)

private def r : Fin k → R := (exists_r ‹_› hg).choose

private lemma hr : ∑ i, r ‹_› hg i * a i = g.constantCoeff := (exists_r ‹_› ‹_›).choose_spec
...
private lemma sum_h_eq_g : ∑ i, h ‹_› ‹_› ‹_› i * f ‹_› i = g := by
  ...
  conv =>
    enter [1, 2, 2, 2, i]
    rw [eq_X_pow_mul_shift_add_trunc (n + 1) (h ‹_› ‹_› ‹_› i), add_comm, add_mul, mul_assoc]
...
theorem exist_eq_span_eq_ncard_of_X_notMem {S : Set R}
  (_ : span S = P.map constantCoeff) (_ : S.Finite) :
...
lemma prime_fg_iff {P : Ideal R⟦X⟧} [P.IsPrime] : P.FG ↔ (P.map constantCoeff).FG := by
  constructor
  · exact (FG.map · _)
  · intro ⟨S, _⟩
    by_cases X ∈ P
    · have H := eq_span_insert_X ‹_› ‹_›
      have : (insert X <| (C (R := R))'' S).Finite := Finite.insert X <| Finite.image _ S.finite_toSet
...
instance [IsNoetherianRing R] : IsNoetherianRing R⟦X⟧ :=
  IsNoetherianRing.of_prime fun P _ ↦
    prime_fg_iff.2 <| (isNoetherianRing_iff_ideal_fg R).1 ‹_› (P.map constantCoeff)
